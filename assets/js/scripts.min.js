...
!function(c){c.fn.simpleJekyllSearch=function(e){var l=c.extend({jsonFile:"/search.json",jsonFormat:"title,tags,categories,url,date,content",template:'<li><article><a href="{url}"><span class="entry-category">{categories}</span> {title} <span class="entry-date"><time datetime="{date}">{date}</time></span></a><p class="search-snippet">{snippet}</p></article></li>',searchResults:".search-results",limit:"10",noResults:"<p>Oh no! We didn't find anything :(</p>"},e),a=l.jsonFormat.split(","),o=[],s=this,i=c(l.searchResults);function r(){i.children().remove()}function g(content,str){var radius=80;if(!content)return"";var idx=content.toLowerCase().indexOf(str.toLowerCase());if(idx===-1)return content.substring(0,radius*2)+"...";var start=Math.max(0,idx-radius);var end=Math.min(content.length,idx+str.length+radius);var snippet=(start>0?"...":"")+content.substring(start,end)+(end<content.length?"...":"");var escaped=str.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");return snippet.replace(new RegExp("("+escaped+")","gi"),"<mark>$1</mark>")}l.jsonFile.length&&i.length&&c.ajax({type:"GET",url:l.jsonFile,dataType:"json",success:function(e,t,n){o=e,s.keyup(function(e){var t,n,s,q;c(this).val().length?(q=c(this).val(),s=[],c.each(o,function(e,t){for(e=0;e<a.length;e++)void 0!==t[a[e]]&&-1!==t[a[e]].toLowerCase().indexOf(q.toLowerCase())&&(s.push(t),e=a.length)}),t=s,r(),i.append(c(l.searchResultsTitle)),t.length?c.each(t,function(e,t){if(e<l.limit){var n=l.template,snip=g(t["content"],q);for(var e=0;e<a.length;e++){var s=new RegExp("{"+a[e]+"}","g");n=n.replace(s,t[a[e]])}n=n.replace(/\{snippet\}/g,snip);i.append(c(n))}}):i.append(l.noResults)):r()})},error:function(e,t,n){console.log("***ERROR in simpleJekyllSearch.js***"),console.log(e),console.log(t),console.log(n)}})}}(Zepto);
