...
!function(p){p.fn.simpleJekyllSearch=function(e){var i=p.extend({jsonFile:"/search.json",jsonFormat:"title,tags,categories,url,date,content",template:'<li><article><a href="{url}"><span class="entry-category">{categories}</span> {title} <span class="entry-date"><time datetime="{date}">{date}</time></span></a><p class="search-snippet">{snippet}</p></article></li>',searchResults:".search-results",limit:"10",noResults:"<p>Oh no! We didn't find anything :(</p>"},e),r=i.jsonFormat.split(","),a=[],s=this,o=p(i.searchResults);function c(){o.children().remove()}i.jsonFile.length&&o.length&&p.ajax({type:"GET",url:i.jsonFile,dataType:"json",success:function(e,t,n){a=e,s.keyup(function(e){var t,l,n,s;p(this).val().length?(n=p(this).val(),s=[],p.each(a,function(e,t){for(e=0;e<r.length;e++)void 0!==t[r[e]]&&-1!==t[r[e]].toLowerCase().indexOf(n.toLowerCase())&&(s.push(t),e=r.length)}),t=s,l=p(this).val(),c(),o.append(p(i.searchResultsTitle)),t.length?p.each(t,function(e,t){if(e<i.limit){for(var n=i.template,s=function(e,t){if(!e)return"";var n=e.toLowerCase().indexOf(t.toLowerCase());if(-1===n)return e.substring(0,160)+"...";var s=Math.max(0,n-80),a=Math.min(e.length,n+t.length+80),l=(0<s?"...":"")+e.substring(s,a)+(a<e.length?"...":""),i=t.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");return l.replace(new RegExp("("+i+")","gi"),"<mark>$1</mark>")}(t.content,l),e=0;e<r.length;e++){var a=new RegExp("{"+r[e]+"}","g");n=n.replace(a,t[r[e]])}n=n.replace(/\{snippet\}/g,s),o.append(p(n))}}):o.append(i.noResults)):c()})},error:function(e,t,n){console.log("***ERROR in simpleJekyllSearch.js***"),console.log(e),console.log(t),console.log(n)}})}}(Zepto);
